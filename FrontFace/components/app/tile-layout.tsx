...
export function TileLayout({ chatOpen }: TileLayoutProps) {
  const {
    ...
    videoTrack: originalAgentVideoTrack,
  } = useVoiceAssistant();
  const [screenShareTrack] = useTracks([Track.Source.ScreenShare]);
  const { localParticipant } = useLocalParticipant();
  const remoteVideoTracks = useTracks([Track.Source.Camera, Track.Source.Unknown], {
    onlySubscribed: true,
  }).filter((t) => t.participant.identity !== localParticipant.identity);

  const agentVideoTrack = originalAgentVideoTrack ?? remoteVideoTracks[0];
